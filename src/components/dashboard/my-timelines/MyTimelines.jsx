...
import { setTimelines, setPagination, resetStoryCardState } from '../../../store/storyCardSlice';
  ...
  const fetchData = async (force = false, page = currentPageFromUrl, append = false) => {
    ...
    try {
      ...
      if (response.status === 429) {
        setIsLoading(false);
        setIsLoadingMore(false);
        toast.warn('Too many requests are made, please relax yourself while using this application');
        return;
      }
      ...
      if (paginationHeader) {
        ...
        dispatch(
          setPagination({
            currentPage: page,
            totalPages: timelines.length > 0 ? Math.ceil(timelines.length / pagination.pageSize) : 1,
            pageSize: pagination.pageSize,
